...
const HorizontalCardProduct = ({category,heading,direction}) => {
    ...
    <div className='container mx-auto px-4 my-6 relative'>
        ...
        loading?(
                ...
                return(
                    <div className='w-full min-w-[300px] md:min-w-[320px] max-w-[300px] md:max-w-[320px] h-40 bg-white rounded-lg shadow flex overflow-hidden'> {/* update (mobile version) max-w-[280px] to max-w-[300px] and min-w-[280px] to min-w-[300px] and h-36 to h-40 */}
                        ...
                        <div className='p-4 grid w-full gap-2'>
                            <h2 className='font-medium text-base md:text-lg text-ellipsis line-clamp-1  text-black bg-slate-200 animate-pulse p-1 rounded-full'><p></p></h2>
                            <p className='capitalize text-slate-500 p-1 bg-slate-200 animate-pulse rounded-full'></p>
                            <div className='pb-1'>
                                <h2 className='font-sm text-green-600 p-2 mb-2 bg-slate-200 animate-pulse rounded-full'><p></p></h2>
                                <div className='flex gap-2 w-full'>
                                    <p className='font-medium text-red-600 p-2 bg-slate-200 w-full animate-pulse rounded-full'></p>
                                    <p className='font-semibold text-slate-500 line-through p-2 bg-slate-200 w-full animate-pulse rounded-full'></p>
                                </div>
                            </div>
                            <button className='text-sm text-white px-3 py-1 rounded-full w-full bg-slate-200 animate-pulse'></button>
                        </div>
                    </div>
                )
            })
            ...
            data.map((product,index)=>{
                ...
                return(
                    <Link to={"product/"+product?._id} className='w-full min-w-[300px] md:min-w-[320px] max-w-[300px] md:max-w-[320px] h-40 bg-white rounded-lg shadow flex overflow-hidden'> {/* update (mobile version) max-w-[280px] to max-w-[300px] and min-w-[280px] to min-w-[300px] and h-36 to h-40 */}
                        ...
                        <div className='p-4 grid'>
                            ...
                            <button className='text-sm custome-bg-orange hover:bg-orange-500 text-white px-3 py-1 rounded-full' onClick={(e)=>handleAddToCart(e,product?._id)}>Add to Cart</button>
                        </div>
                    </Link>
                )
            })
